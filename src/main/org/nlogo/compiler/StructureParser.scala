...
// ExtensionManager gets side effected (it loads and unloads extensions). - ST 2/21/08
import org.nlogo.compiler.CompilerExceptionThrowers.{cAssert,exception}
import org.nlogo.agent.{Agent,Link,Turtle}
import org.nlogo.api.{CompilerException,ErrorSource,ExtensionManager,Let,Program,Token,TokenizerInterface,TokenType}
import org.nlogo.nvm.{Instruction,Procedure}
import org.nlogo.prim._let
private object StructureParser {
  class Results(val procedures:java.util.Map[String,Procedure],val tokens:Map[Procedure,Iterable[Token]])
  ...
  private class TokenBuffer {
    private val tokens = new collection.mutable.ArrayBuffer[Token]
    def slice(i1:Int,i2:Int) = tokens.slice(i1,i2).toList ::: List(Token.eof)
    // iteration
    var index = 0
    def hasNext = index < tokens.size
    def next() = if(hasNext) { index += 1; tokens(index - 1) }
                 else Token.eof
    def head = if(hasNext) tokens(index)
               else Token.eof
    // mutation
...
private class StructureParser(originalTokens:Seq[Token],displayName:Option[String],program:Program,oldProcedures:java.util.Map[String,Procedure],extensionManager:ExtensionManager)(implicit tokenizer:TokenizerInterface) {
  ...
  def parse(subprogram:Boolean):StructureParser.Results = {
    ...
    var index = 1
    usingFiles += (("", null))
    if(!subprogram) usingFiles += (("aggregate", null))
    ...
    while(!totallyDone) {
      ...
      if(index < usingFiles.size) {
        ...
        val source:String =
          ...
          catch{ case _:java.io.IOException => exception("Could not find " + fileName,usingFiles(index)._2) }
        tokenBuffer.appendAll(tokenizer.tokenize(source,fileName))
        index += 1
      }
      else totallyDone = true
    }
    if(!subprogram) extensionManager.finishFullCompilation()
    new StructureParser.Results(newProcedures,Map() ++ tokensMap)
  }
  ...
  private def processTokenWithExtensionManager(token:Token):Token = {
    ...
    if(token.tyype != TokenType.IDENT ||
      ...
      replacement match {
        ...
        case null => token
        case prim =>
          val newType = if(prim.isInstanceOf[org.nlogo.api.Command]) TokenType.COMMAND else TokenType.REPORTER
    ...
    while(!done) {
      ...
      if(!haveTo) {
        ...
        procedure = new Procedure(
          if(isReporterProcedure) Procedure.Type.REPORTER else Procedure.Type.COMMAND,
  ...
  private def parseImport(tokenBuffer:TokenBuffer) {
    ...
    while(!done) {
      ...
      if(token.tyype == TokenType.CLOSE_BRACKET)
        ...
        extensionManager.importExtension(token.value.asInstanceOf[String].toLowerCase,
  ...
  private def parseLet(procedure:Procedure,offset:Int,oldAncestorNames:java.util.List[String]):Let = {
    ...
    while(true) {
      if(!tokenBuffer.hasNext) exception("Expected ] or END",tokenBuffer.head)
      val token = tokenBuffer.head
      if(token.tyype == TokenType.OPEN_BRACKET) {
        level += 1
        tokenBuffer.next()
