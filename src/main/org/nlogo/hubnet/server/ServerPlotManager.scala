...
class ServerPlotManager(workspace: AbstractWorkspaceScala, connectionManager: ConnectionManager,
  ...
  def plot(x: Double, y: Double) {broadcastToClients(PlotPoint(Some(x), y))}
  // Sends the org.nlogo.hubnet.PlotPoint plotted to a single client
  def narrowcastPlot(clientId: String, y: Double) {sendToClient(clientId, PlotPoint(None, y))}
  // Sends the org.nlogo.hubnet.PlotPoint plotted to a single client
  def narrowcastPlot(clientId: String, x: Double, y: Double) {sendToClient(clientId, PlotPoint(Some(x), y))}
