...
class ServerPlotManager(workspace: AbstractWorkspaceScala, connectionManager: ConnectionManager,
  ...
  def initPlotListeners() = for(plot <- plots){
  ...
  def broadcastPlots() = for(p<-plots; if (isBroadcast(p.name)))
    connectionManager.broadcast(p)

  // called when a new client logs in and needs to be brought up to date
  def sendPlots(client:String) = if (broadcastEnabled) for(p<-plots; if (isBroadcast(p.name)))
