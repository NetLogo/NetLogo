...
class ClientRenderer(world: ClientWorld) extends AbstractRenderer(world, new ShapeList, new ShapeList) {
  ...
  override def paintTurtles(g: GraphicsInterface, patchSize: Double) = for(data <- world.getTurtles.asScala)
    turtleDrawer.drawTurtle(g, topology, data, patchSize)
  override def paintLinks(g: GraphicsInterface, patchSize: Double) = for(data <- world.getLinks.asScala)
    linkDrawer.drawLink(g, topology, data, patchSize, false)
  override def paintPatchLabels(g: GraphicsInterface, patchSize: Double) = for(data <- world.getPatches)
    if(data.hasLabel) drawPatchLabel(g, data, patchSize)
  ...
  def stamp(turtle: HubNetTurtleStamp) = trailDrawer.stamp(new TurtleData(turtle), turtle.erase)
  def stamp(link: HubNetLinkStamp) = trailDrawer.stamp(new LinkData(link), link.erase)
  def clearDrawing() = trailDrawer.clearDrawing()
