...
import java.awt.{FlowLayout, Dimension, BorderLayout, Frame}
import java.awt.event.{ActionEvent, MouseEvent, MouseAdapter, ActionListener}
import table.{AbstractTableModel, DefaultTableCellRenderer}
import org.nlogo.hubnet.connection.ClientRole

abstract class LoginCallback{
  def apply(user:String, host:String, port:Int, role:ClientRole)
}
...
//      add(controllerRadioButton)
  ...
  def actionPerformed(e: ActionEvent) {
    try this.loginCallback(username, server, port, role)
    catch {
