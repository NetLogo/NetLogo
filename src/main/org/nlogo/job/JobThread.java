	...
	private void runPrimaryJobs() {
		for( int i = 0 ; i < primaryJobs.size() ; i++ ) {
			...
			if( job.state != Job.RUNNING ) {
				...
				if( job.isTurtleForeverButtonJob() ) { turtleForeverButtonJobs.remove( job ) ; }
				if( job.isLinkForeverButtonJob() ) { linkForeverButtonJobs.remove( job ) ; }
				job.state = Job.REMOVED ;
				if( job.topLevelProcedure != null ) {
					jobManagerOwner.updateDisplay( false ) ;
					synchronized( job ) { job.notifyAll() ; }
				...
				try { synchronized( lock ) { job.step() ; } }
