...
import org.nlogo.api.{I18N, AgentKind, Editable, ModelReader, Options, Version}
...
object ButtonWidget {
  ...
  object ButtonType {
    ...
    private def apply(headerCode:String, kind: AgentKind, imagePath: String): ButtonType = {
      val img = image(imagePath)
      new ButtonType(headerCode, kind, Some(img), Some(darkImage(img)))
    }
    def apply(kind: AgentKind): ButtonType = {
      buttonTypes.find(_.kind == kind).getOrElse(ObserverButton) //TODO or should we say error("bad agent class")
    }
    ...
    def getKind(name: String) = {
      if(name == "NIL") ObserverButton.kind
      //TODO or should we say error("bad agent name")
      buttonTypes.find(_.name == name).map(_.kind).getOrElse(ObserverButton.kind)
    }
  ...
  case class ButtonType(name: String, kind: AgentKind,
