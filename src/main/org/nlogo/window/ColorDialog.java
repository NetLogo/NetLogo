...
WindowListener
{
	...
	public Color showPlotPenDialog(Color initialColor)
	{
		...
		if (okCancelFlag == 1) {return selectedColor ; }
		else { return null;}	
	}
	...
	private void selectSwatch(Color initialColor)
	{
		 ...
		 if (closest % 1 == 0 || closest >= 9.9)  { oneStep.doClick(); }
		 else if (closest % 5 == 0)  { pointOneStep.doClick(); }		 
		 else  { pointFiveStep.doClick(); }		  
		 ...
		 for( Enumeration<javax.swing.AbstractButton> e = swatchesGroup.getElements()  ; e.hasMoreElements() ; )
		 {
			 ...
			 if ( closestColor.equals(color) ) { swatch.doClick(); }
		 }	
	}
	...
	private void createSwatches () 
	{
		...
		for(int i=0; i<rows; i++)
		{
			...
			for(int j=0; j < (swatchPerRow + 1) ; j++)
			{
				...
				if (j >= swatchPerRow) { colorNumber = (i * 10) +9.9 ;}
				else {swatch.setForeground(Color.BLACK);}
