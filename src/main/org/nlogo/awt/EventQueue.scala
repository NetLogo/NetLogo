...
object EventQueue {
  ...
  def invokeLater(r: Runnable) = java.awt.EventQueue.invokeLater(r)
  ...
  def invokeAndWait(r: Runnable) = java.awt.EventQueue.invokeAndWait(r)

  def mustBeEventDispatchThread() = require(isDispatchThread, "not event thread: " + Thread.currentThread)

  def cantBeEventDispatchThread() = require(!isDispatchThread)
