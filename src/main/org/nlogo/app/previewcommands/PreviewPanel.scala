...
class PreviewPanel(graphicsPreview: GraphicsPreviewInterface) extends JPanel {
  ...
  def wrap(ps: String*) = ps
    .map("<p style='text-align:center; margin:10px'>" + escape(_) + "</p>")
    .mkString("<html>", "", "</html>")
  def executeCommandsAction(runnableCommands: Option[RunnablePreviewCommands]) =
    new AbstractAction("Run Preview Commands") {
      def run(): Unit = {
        runnableCommands match {
          case Some(runnable) =>
            ModalProgressTask(getFrame(PreviewPanel.this), "Running Preview Commands", runnable, is3D)
