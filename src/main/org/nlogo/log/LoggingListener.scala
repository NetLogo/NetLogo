...
trait LoggingListener extends NetLogoListener {
  ...
  override def chooserChanged(name: String, value: AnyRef, valueChanged: Boolean) = if (valueChanged) {
      chooserMsg.updateGlobalMessage(name.toUpperCase, value.toString)
      Greens.info(chooserMsg)
    }
  override def inputBoxChanged(name: String, value: AnyRef, valueChanged: Boolean) = if (valueChanged) {
