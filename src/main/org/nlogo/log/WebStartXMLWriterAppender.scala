...
class WebStartXMLWriterAppender(mode: LogSendingMode, destinations: URL*)
  ...
  /*none*/
  def initialize() {
    initializeTransformer(destinations.mkString("|"), writer)
  }

  override def close() {
    closeDocument(); super.close()
  }
}
...
class RemoteLogWriter(mode: LogSendingMode, destinations: URL*) extends Writer {
  ...
  override def write(cbuf: Array[Char], off: Int, len: Int) {
    director ! ToDirectorWrite(cbuf.subSequence(off, off + len).toString)
  }
  ...
  /*none*/
  def delete() {
    director !? ToDirectorAbandon
  }

  override def close() {
    director !? ToDirectorFinalize
