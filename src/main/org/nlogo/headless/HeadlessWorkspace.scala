...
import org.nlogo.api.{WidgetIO, JobOwner, Version, RendererInterface, Perspective, WorldDimensions, WorldDimensions3D, AggregateManagerInterface, ModelReader, CompilerException, LogoException, SimpleJobOwner, HubNetInterface, CommandRunnable, ReporterRunnable}
...
with org.nlogo.api.ViewSettings {
  ...
  private var _serverWidgetSpecs: Iterable[WidgetIO.WidgetSpec] = Nil
  def serverWidgetSpecs = _serverWidgetSpecs
  def loadServerInterface(specs: Iterable[WidgetIO.WidgetSpec]) { _serverWidgetSpecs = specs }
