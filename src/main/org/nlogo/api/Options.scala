...
class Options[T] {
  ...
  def addOption(name: String, value: T): Unit = choices += Option(name, value)
  ...
  def selectByName(s: String) = current = choices.find(_.name == s)

  def selectValue(obj: T) = current = choices.find(_.value == obj)
