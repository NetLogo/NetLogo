...
class SetVisitorTests extends FunSuite {
  def tester(r: Reporter, spec: String, setterClass: Class[_ <: Command]) {
    ...
    stmt.addArgument(new ReporterApp(new _constdouble(java.lang.Double.valueOf(5)), 0, 0, ""))
    stmt.accept(new SetVisitor)
    expect(setterClass.getName.substring("org.nlogo.prim.".length) + ":" + spec + "[_constdouble:5.0[]]")(
      stmt.toString)
  }
  test("test1") { tester(new _turtlevariable(Turtle.VAR_HEADING), String.valueOf(Turtle.VAR_HEADING), classOf[_setturtlevariable]) }
  ...
  test("test4") { tester(new _patchvariable(Patch.VAR_PCOLOR), String.valueOf(Patch.VAR_PCOLOR), classOf[_setpatchvariable]) }
  ...
  test("test8") { tester(new _turtlevariable(Turtle.VAR_HEADING), String.valueOf(Turtle.VAR_HEADING), classOf[_setturtlevariable]) }
  test("test9") { tester(new _breedvariable("FOO"), "FOO", classOf[_setbreedvariable]) }
  test("test10") { tester(new _linkvariable(Link.VAR_THICKNESS), String.valueOf(Link.VAR_THICKNESS), classOf[_setlinkvariable]) }
  test("not found") {
