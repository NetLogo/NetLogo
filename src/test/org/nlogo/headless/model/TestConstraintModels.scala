...
class TestConstraintModels extends FunSuite with Fixture {

  test("Boolean Constraint Constructor") {
    runModel(Model(widgets =
  ...
  test("Chooser Constraint") {
    runModel(Model(widgets = List(
  ...
  test("InputBox Constraint Loading") {
    runModel(Model(widgets = List(
  ...
  test("Slider Constraints") {
    runModel(Model(widgets = List(Slider(name="x-loc", min="0", max="100", current="10", inc="1")))) {
      reporter("x-loc") -> 10d
