...
class TestPlotModels extends FunSuite with Fixture {
  ...
  test("plot on tick") { runModel(theModel) {
    observer>>"setup"
    observer>>"go"
  ...
  test("several ticks") { runModel(theModel) {
    observer>>"setup"
    observer>>"tick"
  ...
  test("update-plots") { runModel(theModel) {
    observer>>"setup"
  ...
  test("plot with setup code and pen with setup code") {
    runModel(Model(modelCode, widgets = List(
      Plot(setupCode = "create-dogs 5",
  ...
  test("pen with no update code should not get plotted on tick") {
    runModel(Model(modelCode, widgets = List(Plot(pens = Pens(Pen(updateCode = "")))))) {
  ...
  test("plot update code should run on tick") { runModel(
  ...
  test("two plots with setup code") { runModel(
    Model(modelCode, widgets = List(
  ...
  test("stop in plot update code") { runModel(
    Model(modelCode, widgets = List(
  ...
  test("stop in plot update code doesnt kill outer procedure") { runModel(
  ...
  test("stop in plot update code doesnt kill outer procedure (2)") { runModel(
  ...
  test("inner stop doesnt prevent pens from running") { runModel(
    Model(modelCode, widgets = List(
  ...
  test("stop in pen doesnt prevent other pens from running") { runModel(
    Model(modelCode, widgets = List(
  ...
  test("plot code uses aux rng") { runModel(
  ...
  def testCompileError(model: Model)(f: Throwable => Unit) = {
  ...
  test("Plot With Bad Update Code Should Throw Exception on Load (headless only)") {
  ...
  test("Plot With Bad Setup Code Should Throw Exception on Load (headless only)") {
    testCompileError(Model(modelCode, widgets = List(Plot(setupCode="weijefwef")))){ ex =>
      assert("Nothing named WEIJEFWEF has been defined" === ex.getMessage)
    }}

  test("Plot With Bad Pen Setup Code Should Throw Exception on Load (headless only)") {
  ...
  test("Plot With Bad Pen Update Code Should Throw Exception on Load (headless only)") {
