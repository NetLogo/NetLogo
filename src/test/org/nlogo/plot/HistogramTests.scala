...
class HistogramTests extends FunSuite {
  test("empty histogram") {
    ...
    expectResult(0)(histogram.ceiling)
    expectResult(List(0, 0, 0))(histogram.bars.toList)
  }
  test("integer histogram") {
    val histogram = new Histogram(0, 3, 1)
    List(0.0, 0.5, 0.999999999, 1.0, 1.999999999, 2.0).foreach(histogram.nextValue)
    expectResult(3)(histogram.ceiling)
    expectResult(List(3, 2, 1))(histogram.bars.toList)
  }
  ...
  test("floating point pitfall") {
    ...
    expectResult(1)(histogram.ceiling)
    expectResult(10)(histogram.bars.size)
  ...
  test("small range") {
    ...
    expectResult(1)(histogram.ceiling)
    expectResult(List(1, 1, 1, 0, 0))(histogram.bars.toList)
  }
}
