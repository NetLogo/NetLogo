...
class ProtocolEditableTests extends FunSuite {
  val compiler = Femto.scalaSingleton(classOf[ParserInterface],
  ...
  // just by beefing up DummyParserServices a little, since all we're using the compiler for is to
  ...
  test("round trip") {
    val workspace = new DummyParserServices {
