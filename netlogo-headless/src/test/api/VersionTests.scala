...
class VersionTests extends AnyFunSuiteEx {
  ...
  test("currentVersion") {
    ...
    assert(compatibleVersion("NetLogo 6.4"))
    assert(compatibleVersion("NetLogo 6.4RC1"))
    assert(compatibleVersion("NetLogo 6.4beta1"))
    assert(compatibleVersion("NetLogo 6.4pre1"))
    assert(compatibleVersion("NetLogo 6.4alpha1"))
    assert(compatibleVersion("NetLogo 6.4.1"))
    assert(compatibleVersion("NetLogo 6.4.1weirdversion"))
    assert(compatibleVersion("NetLogo 6.4weirdversion"))
  }
  test("futureMinor") {
  ...
  if (buildDate != "INTERIM DEVEL BUILD") test("Date") {
    ...
    val format = new SimpleDateFormat("MMMM d, yyyy", Locale.US)
    val date = format.parse(buildDate)
    assertResult(format.format(date))(buildDate)
    assert(date.after(new SimpleDateFormat("y", Locale.US).parse("1998")))
    assert(date.before(new SimpleDateFormat("y", Locale.US).parse("2100")))
  }
