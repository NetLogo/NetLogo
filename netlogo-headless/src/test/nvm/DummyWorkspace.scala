...
class DummyWorkspace extends Workspace {
  ...
  def breathe(context: org.nlogo.nvm.Context): Unit = unsupported
  def inspectAgent(agent: org.nlogo.agent.Agent,radius: Double): Unit = unsupported
  def parser: org.nlogo.core.LiteralParser = unsupported
  def procedures_=(procedures: org.nlogo.nvm.Procedure.ProceduresMap): Unit = unsupported
  def requestDisplayUpdate(context: org.nlogo.nvm.Context,force: Boolean): Unit = unsupported
}
