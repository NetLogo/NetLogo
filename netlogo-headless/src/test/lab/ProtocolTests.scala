...
class ProtocolTests extends FunSuite {
  ...
  test("enumerated 2") {
    val protocol =
      new LabProtocol("enumerated2", "", "", "", 2, true, 0, "", Nil,
                   List(new EnumeratedValueSet("foo", List(1d, 2d, 3d))))
  ...
  test("avoid floating point error") {
    val protocol = new LabProtocol("stepped3", "", "", "", 1, true, 0, "", Nil,
  ...
  test("both") {
    def make(repetitions:Int) =
      new LabProtocol("both", "", "", "", repetitions, true, 0, "", Nil,
                   List(new EnumeratedValueSet("foo", List(1d, 2d, 3d)),
