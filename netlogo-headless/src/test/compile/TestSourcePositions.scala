...
import org.nlogo.nvm.{ CompilerFlags, Procedure},
  Procedure.NoProcedures
...
class TestSourcePositions extends FunSuite {
  ...
  def compileReporter(source: String) =
    Compiler.compileMoreCode("to foo __ignore " + source + "\nend", None, program,
      NoProcedures,
      new DummyExtensionManager, new DummyCompilationEnvironment,
      CompilerFlags()).head.code.head.args.head.fullSource
  def compileCommand(source: String) =
    Compiler.compileMoreCode("to foo " + source + "\nend", None, program,
      NoProcedures,
      new DummyExtensionManager, new DummyCompilationEnvironment,
      CompilerFlags()).head.code.head.fullSource
