...
class Compiler(dialect: Dialect) extends PresentationCompilerInterface {
  ...
  override def checkCommandSyntax(source: String, program: Program, procedures: ListMap[(String, Option[String]), Procedure],
  ...
  override def checkReporterSyntax(source: String, program: Program, procedures: ListMap[(String, Option[String]), Procedure],
                                   extensionManager: ExtensionManager, parse: Boolean,
                                   compilationEnv: CompilationEnvironment): Unit = {
  ...
  private def checkSyntax(source: String, program: Program, procedures: ListMap[(String, Option[String]), Procedure],
                          extensionManager: ExtensionManager, parse: Boolean,
                          compilationEnv: CompilationEnvironment): Unit = {
    frontEnd.frontEnd(source, None, program, true, ListMap[(String, Option[String]), Procedure](procedures.toSeq*),
                      extensionManager)
  }
  ...
  override def isReporter(source: String, program: Program, procedures: ListMap[(String, Option[String]), Procedure],
                          extensionManager: ExtensionManager, compilationEnv: CompilationEnvironment): Boolean =
    utilities.isReporter(source, program, ListMap[(String, Option[String]), Procedure](procedures.toSeq*), extensionManager)
