...
object JFlexRunner {
  ...
  def flex(log: String => Unit, autogenFolder: File, cacheDirectory: File, srcRoot: File, ppackage: String, kind: String): File = {
    // when we move to JFlex 1.6+, --noinputstreamctor can be removed
    jflex.Main.main(Array("--quiet", "--noinputstreamctor", "-d", cacheDirectory.getPath, (autogenFolder / "flex" / (kind + ".flex")).asFile.getAbsolutePath.toString))
