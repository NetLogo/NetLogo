...
object PackageWinAggregate {
  ...
  , launchers: Seq[Launcher]
    ...
    val launcherExes = launchers.map( (launcher) => s"${launcher.name}.exe" ).toSet - "NetLogo Console.exe"
    val generatedUUIDs =
    ...
    NetLogoPackaging.generateChecksums(log, version, arch, appImageDir.toPath, webDir.getParentFile.toPath)
