...
object FileActions {
  ...
  // Creates a new directory specified as a file
  // The createDirectories method should be used if it is necessaryto create
  // all nonexistent parent directories first.
  // No error if the directory already exists
  def createDirectory(f: File): Unit =
    createDirectory(f.toPath)

  // Creates a new directory specified as a path
  // The createDirectories method should be used if it is necessary to create
  // all nonexistent parent directories first.
  // No error if the directory already exists
  ...
  // Copies a path to a destination, overwriting the target path if it exists
  // If file is a directory, does not copy contents.
  ...
  // Copies a directory (as file) and its contents to a destination,
  // overwriting any target files that exist
  def copyDirectory(src: File, dest: File): Unit = {
    copyDirectory(src.toPath, dest.toPath)
  }

  // Copies a directory (as path) and its contents to a destination,
  ...
  // Copies a directory (as path) and its contents that satisfy a filter
  // to a destination, overwriting any target files that exists
  ...
  // Moves the file to the target file, failing if the target file exists
  // Moving a directory will fail if it requires moving sub files.
