  ...
  def openFromMap(map: ModelReader.ModelMap) {
    ...
    val results = {
      ...
      ws.compiler.compileProgram(
        code, Program.empty(Version.is3D).copy(
    ...
    val previewCommandsSource = map(ModelSection.PreviewCommands).mkString("\n")
  ...
  private object WidgetParser {
    ...
    def parseWidgets(widgetsSection: Seq[String], netLogoVersion: String) = {
      ...
      def parsePlot(widget: Seq[String]) {
        ...
        // JC - 9/14/10
        PlotLoader.parsePlot(widget.toArray, ws.plotManager.newPlot(""), identity)
      }
