...
import org.nlogo.{ prim, nvm, parse }
...
private class SimpleOfVisitor extends parse.DefaultAstVisitor {
  override def visitReporterApp(app: parse.ReporterApp) {
    if(app.reporter.isInstanceOf[prim._of])
      process(app)
    super.visitReporterApp(app)
  }
  private def process(app: parse.ReporterApp) {
    for(r <- convert(app.args(0).asInstanceOf[parse.ReporterBlock].app.reporter)) {
