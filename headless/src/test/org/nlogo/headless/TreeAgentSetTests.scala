...
class TreeAgentSetTests extends FunSuite with GivenWhenThen with TestUsingWorkspace {
  ...
  testUsingWorkspace("TreeAgentSet should trigger SimpleChangeEvent") { ws =>
    ...
    expectResult(1)(turtlesSub.eventCount)
    And("other subscribers should get none")
    for (s <- allSubs if s != turtlesSub) expectResult(0)(s.eventCount)
    ...
    expectResult(2)(turtlesSub.eventCount)
    And("other subscribers should get none")
    for (s <- allSubs if s != turtlesSub) expectResult(0)(s.eventCount)
    ...
    expectResult(1)(miceSub.eventCount)
    And("other subscribers should get none")
    for (s <- allSubs if !Set(turtlesSub, miceSub).contains(s)) expectResult(0)(s.eventCount)
    ...
    expectResult(2)(miceSub.eventCount)
    And("other subscribers should get none")
    for (s <- allSubs if !Set(turtlesSub, miceSub).contains(s)) expectResult(0)(s.eventCount)
    ...
    expectResult(1)(linksSub.eventCount)
    And("other subscribers should get none")
    for (s <- allSubs if s != linksSub) expectResult(0)(s.eventCount)
    ...
    expectResult(1)(linksSub.eventCount)
    And("other subscribers should get none")
    for (s <- allSubs if !Set(linksSub, undirLinksSub).contains(s)) expectResult(0)(s.eventCount)
  }
}
