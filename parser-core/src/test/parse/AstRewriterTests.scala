...
class AstRewriterTests extends FunSuite {
  ...
  def removeFirstArg(source: String, dropArgumentFrom: String): String = {
    trimmedRewriteCommand(source, _.removeFirstArg(dropArgumentFrom))
  }
  ...
  test("remove argument") {
    ...
    assertResult("show patch 1")(removeFirstArg("show patch 0 1", "patch"))
    assertResult("show")(removeFirstArg("show \"abc\"", "show"))
    assertResult("show \"abc\"")(removeFirstArg("show \"abc\"", "clear-all"))
