...
import org.nlogo.core.{ Breed, CompilationEnvironment, CompilationOperand, CompilerException, ErrorSource, I18N, ProcedureSyntax, Program, StructureResults, Token, TokenizerInterface, TokenType }
...
object StructureParser {
  ...
  private def prefixChangedProcedures(oldProcedures: ProceduresMap, newProcedures: ProceduresMap, prefix: String): ProceduresMap = {
    val changedProcedures = newProcedures.drop(oldProcedures.size).map{case (name, proc) =>
      ...
      val newToken = decl.name.token.copy(text = prefix + proc.name, value = prefix.toUpperCase + decl.name.token.value)
