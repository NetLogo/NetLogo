...
object StructureConverter {
  ...
  def convert(declarations: Seq[Declaration],
    ...
    val ims = declarations.collect {
      case l: ImportDecl =>
        val tokenFilename = l.token.sourceLocation.filename
        val filename = if (tokenFilename.isEmpty()) None else Some(tokenFilename)

        Import(filename, l.pathComponents, l.pathAlias, l.importedIdentifiers, l.token)
    }
  ...
  def updateProgram(program: Program, declarations: Seq[Declaration]): Program = {
    def updateVariables(program: Program): Program =
      declarations.foldLeft(program) {
        case (program, Variables(Identifier("GLOBALS", _), identifiers, _, _)) =>
          program.copy(userGlobals = program.userGlobals ++ identifiers.map(_.name))
        case (program, Variables(Identifier("TURTLES-OWN", _), identifiers, _, _)) =>
        ...
        case (program, Variables(Identifier("PATCHES-OWN", _), identifiers, _, _)) =>
        ...
        case (program, Variables(Identifier("LINKS-OWN", _), identifiers, _, _)) =>
        ...
        case (program, Variables(Identifier(breedOwn, tok), identifiers, _, _)) =>
    ...
    def updateBreeds(program: Program): Program =
      declarations.foldLeft(program) {
        case (program, Breed(plural, singular, isLinkBreed, isDirected, _, _)) =>
