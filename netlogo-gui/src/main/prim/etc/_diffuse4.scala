...
import org.nlogo.nvm.{ Command, Context, EngineException, Referencer }

class _diffuse4 extends Command with Referencer {
  switches = true
  ...
  override def perform(context: Context): Unit = {
    ...
    if (diffuseparam < 0.0 || diffuseparam > 1.0) {
      throw new EngineException(context, this, I18N.errors.getN("org.nlogo.prim.$common.paramOutOfBounds", Double.box(diffuseparam)))
    }
    try {
      world.diffuse4(diffuseparam, reference.vn)
    } catch {
      case e: AgentException                => throw new EngineException(context, this, e.getMessage())
      case e: UnsupportedOperationException => throw new EngineException(context, this, "Diffuse4 is not supported in 3D")
      case e: PatchException                =>
        ...
        throw new EngineException(context, this, message)
    }
    context.ip = next;
  }
}
