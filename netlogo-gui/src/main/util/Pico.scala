...
class Pico extends DefaultPicoContainer(new Caching)
{
  ...
  def add(key: Class[?], name: String, parameters: Array[Parameter]): Unit = {
    addComponent(key, Class.forName(name), parameters*)
  }
  def add(key: Class[?], name: String, parameters: Parameter*): Unit = {
    addComponent(key, Class.forName(name), parameters*)
  }
  def add(key: Class[?], name: String): Unit = {
