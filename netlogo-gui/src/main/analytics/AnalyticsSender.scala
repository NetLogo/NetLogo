...
object AnalyticsSender {
  ...
  private def sendCache(): Unit = {
    if (Files.exists(cachePath) && !Files.isDirectory(cachePath)) {
      Files.readAllLines(cachePath).forEach(_ match {
        case s"$tpe,$payload" =>
          tpe.toIntOption.foreach { ord =>
            Future(send(AnalyticsEventType.fromOrdinal(ord), Some(payload.trim).filter(_.nonEmpty)))
          }
      })
