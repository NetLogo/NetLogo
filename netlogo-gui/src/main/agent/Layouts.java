...
public final strictfp class Layouts {
  ...
  public static void spring(World world, AgentSet nodeset, AgentSet linkset,
    ...
    if (world.program().dialect().is3D()) {
      spring3D((World3D) world, nodeset, linkset, spr, len, rep, random);
    } else {
  ...
  public static void radial(World world, AgentSet nodeset, AgentSet linkset, Turtle root)
    ...
    while (!queue.isEmpty()) {
      ...
      lastNode = node;
      for (Iterator<Turtle> neighbors =
           JavaConversions.asJavaIterator(linkManager.findLinkedWith(node.val, linkset));
           neighbors.hasNext();) {
        Turtle t = neighbors.next();
