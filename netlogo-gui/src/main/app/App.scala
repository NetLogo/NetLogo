    ...
    while (iter.hasNext) {
      iter.next match {
        ...
        case "--color-theme" =>
          ...
          val known = next == "classic" || next == "light" || next == "dark" || next == "system"

          require(known, I18N.errors.getN("themes.unknown", next))
...
class App(args: App.CommandLineArgs) extends LinkChild with Exceptions.Handler with AppEvent.Handler
  ...
  private lazy val editDialogFactory = new EditDialogFactory(colorizer)
  ...
  private val interfaceFactory = new InterfaceFactory {
    def widgetPanel(frame: Frame, workspace: GUIWorkspace, widgetInfos: Seq[WidgetInfo]): AbstractWidgetPanel =
      new WidgetPanel(frame, workspace, widgetInfos, editDialogFactory)
  }
  ...
  private val interfaceTab: InterfaceTab = new InterfaceTab(workspace, monitorManager, editDialogFactory,
                                                            new CommandCenter(workspace, true, interfaceTab.sizeToFit))
  ...
  val dirtyMonitor = new DirtyMonitor(frame, modelSaver, modelLoader, workspace,
                                      _.fold(modelTitle())(externalFileTitle), tabManager)
  ...
  locally {
    ...
    OsThemeDetector.getDetector.registerListener { dark =>
      if (NetLogoPreferences.get("colorTheme2", NetLogoPreferences.get("colorTheme", "system")) == "system") {
        if (dark) {
