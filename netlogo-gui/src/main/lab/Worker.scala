    ...
    extends Callable[Unit]
  {
    ...
    def callHelper(ws: Workspace) {
      ...
      def setVariables(settings: List[(String, Any)]) {
        ...
        if(d.minPxcor >= d.maxPxcor || d.minPycor >= d.maxPycor)
          throw new FailedException("Maximum x and y coordinates should be greater than the minimum coordinates")
        if(d.maxPxcor < 0 || d.maxPycor < 0) 
          throw new FailedException("Maximum x and y coordinates should be positive")
