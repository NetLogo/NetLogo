...
class TestBehaviorSpace
  ...
  def newWorkspace(is3D: Boolean) = {
    val w = HeadlessWorkspace.newInstance(is3D)
    workspaces += w
    w
  }
  ...
  def run2DExperiment(minX: Int, maxX: Int, minY: Int, maxY: Int, declarations: String, name: String)(r: Runner) = {
    import r._
    workspace.initForTesting(minX, maxX, minY, maxY, declarations)
    run("test/lab/" + name)(() => workspace, () => newWorker(name, r))
    workspace
  }
  def runExperiment(worldSize: Int, declarations: String, name: String)(r: Runner) = {
    import r._
    initForTesting(worldSize, declarations)
    run("test/lab/" + name)(() => workspace, () => newWorker(name, r))
    workspace
  }
  def run3DExperiment(name: String)(r: Runner) = {
    import r._
    initForTesting(0)
    run("test/lab/" + name)(() => workspace, () => newWorker(name, r))
  }

  def runParallelExperiment(name: String, declarations: String = "")(r: Runner) {
    import r._
    def workspace = {
      val w = newWorkspace(instance.version.is3D)
      w.initForTesting(instance.version.is3D, 0, declarations)
      w
    }
    // only get spreadsheet results, since parallel table results are in scrambled order - ST 3/4/09
    run("test/lab/" + name, wantTable = false, threads = Runtime.getRuntime.availableProcessors)(
        workspace _, () => newWorker(name, r))
  }
  ...
  testInSpace("BehaviorSpace1", normalConfigurations)(
    runExperiment(0, "globals [param1 param2 counter]", "testBehaviorSpace1"))

  testInSpace("BehaviorSpace2", normalConfigurations)(
    runExperiment(0, "globals [param1 param2 counter]", "testBehaviorSpace2"))

  testInSpace("MultipleMetrics", normalConfigurations)(
    runExperiment(0, "globals [counter param1]", "testMultipleMetrics"))

  testInSpace("NoMetrics1", normalConfigurations)(
    runExperiment(0, "globals [counter param1]", "testNoMetrics1"))

  testInSpace("NoMetrics2", normalConfigurations)(
    runExperiment(0, "globals [counter param1]", "testNoMetrics2"))

  testInSpace("ImmediateExit", normalConfigurations) { r =>
    val workspace =
      runExperiment(0, "globals [counter foo]", "testImmediateExit")(r)
    assertResult(Double.box(99))(workspace.report("foo"))
  }

  testInSpace("CarryoverBetweenRuns", normalConfigurations) { r =>
    import r._
    initForTesting(0, "globals [foo]")
    // no setup commands, so foo doesn't get reset
    newWorker("testCarryover", r).run(workspace, () => workspace, 1)
    assertResult(Double.box(20))(workspace.report("foo"))
  }

  testInSpace("ResizingWorld1", normalConfigurations)(
    runExperiment(0, "", "testResizingWorld1"))

  testInSpace("ResizingWorld2", normalConfigurations)(
    runExperiment(0, "", "testResizingWorld2"))

  testInSpace("SettingRandomSeed", normalConfigurations)(
    runExperiment(0, "", "testRandomSeed"))

  testInSpace("RunNumber", normalConfigurations) { r =>
    val workspace = runExperiment(0, "", "runNumber")(r)
    ...
    assertResult(Double.box(3))(workspace.report("behaviorspace-run-number"))
  }

  testInSpace("ExperimentName", normalConfigurations) { r =>
    val workspace = runExperiment(0, "", "runNumber")(r)
    assertResult("runNumber")(workspace.report("behaviorspace-experiment-name"))
  }
  ...
  testInSpace("ResizingWorld3", normalTwoD) { r =>
    run2DExperiment(0, 1, 0, 1, "", "testResizingWorld3")(r)
  }

  testInSpace("Stopping1", normalConfigurations)(
    runExperiment(0, "globals [x]", "testStopping1"))

  testInSpace("Stopping2", normalConfigurations)(
    runExperiment(0, "globals [x] to go if x = 5 [ stop ] set x x + 1 end", "testStopping2"))

  testInSpace("DontRunMetricsAtEveryStep", normalConfigurations)(
    runExperiment(0,
  ...
  testInSpace("metricsLocalRandomness", normalConfigurations)(
    runExperiment(0, "globals [x]", "metricsLocalRandomness"))

  testInSpace("exitConditionLocalRandomness", normalConfigurations)(
    runExperiment(0, "globals [x]", "exitConditionLocalRandomness"))

  testInSpace("metricGoBoom", normalConfigurations)(
  ...
  testInSpace("metricGoBoom2", normalConfigurations)(runExperiment(0, goBoom2Declarations, "metricGoBoom2"))
  ...
  testInSpace("setupCommandsGoBoom", normalConfigurations)(
    runExperiment(0, "", "setupCommandsGoBoom"))

  testInSpace("goCommandsGoBoom", normalConfigurations)(
    runExperiment(0, "", "goCommandsGoBoom"))

  testInSpace("metricsWithSideEffects", normalConfigurations)(
    runExperiment(0,
      "globals [g] to-report metric set g g + 1 report g end",
      "metricsWithSideEffects"))

  testInSpace("ResizingWorld13d", normalThreeD)(
    run3DExperiment("testResizingWorld13d"))

  testInSpace("ResizingWorld23d", normalThreeD)(
  ...
  */
  testInSpace("dontRunMetricsIfNoListener", normalConfigurations) { r =>
    import r._
    initForTesting(0)
    newWorker("metricGoBoom", r).run(workspace, () => workspace, 1)
    // with no output being generated, the metrics shouldn't be run at all,
    // so no runtime error
    assertResult(Double.box(2))(workspace.report("ticks"))
  }
}
