...
class OpenModelTests extends AnyFunSuite {
  ...
  trait OpenTest {
    ...
    def currentVersion = "NetLogo 6.4"
  ...
  test("if the model is 3D, but NetLogo is open in 2D mode, notifies the user") { new OpenTest {
    override def modelChanges = _.copy(version = "NetLogo 3D 6.4")
    userContinuesOpen()
    assert(openedModel.isDefined)
    assert(controller.notifiedModelArity == 3)
    assert(controller.notifiedModelVersion == "NetLogo 3D 6.4")
  ...
  test("if the model is in 2D, but NetLogo is open in 3D, notifies the user") { new OpenTest {
    override def currentVersion = "NetLogo 3D 6.4"
    userContinuesOpen()
    assert(openedModel.isDefined)
    assert(controller.notifiedModelArity   == 2)
    assert(controller.notifiedModelVersion == "NetLogo 6.4")
  } }
  ...
  test("serializes various version in the model") { new OpenTest {
    assert(nlogoformat.version.serialize(new Model()) === Array[String]("NetLogo 6.4"))
    assert(nlogoformat.version.serialize(new Model(version = "NetLogo 3D 6.4")) ===
      Array[String]("NetLogo 3D 6.4"))
  } }
