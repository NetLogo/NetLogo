...
class JarLoaderTests extends AnyFunSuiteEx with BeforeAndAfter {
  ...
  val dummyExtensionData =
    new ExtensionData("dummy", dummyJarURL, "dummy", "org.nlogo.workspace.DummyClassManager", Some("7.1"), 0)

  val dummyManifest = {
    val m = new JarManifest
    val attrs = m.getMainAttributes
    attrs.putValue("NetLogo-Extension-API-Version", "7.1")
  ...
  test("extensionData returns ExtensionData when it succeeds in connecting to a jar") {
    ...
    assert(extensionData.version == Some("7.1"))
