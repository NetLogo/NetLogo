...
import org.nlogo.api.{ RefEnumeratedValueSet, LabProtocol, SteppedValueSet, ValueList }
...
class ProtocolTests extends AnyFunSuite {
  ...
  test("enumerated 2") {
    val protocol =
      new LabProtocol("enumerated2", "", "", "", 2, true, true, "", 0, "", Nil, ValueList(valueSet123))
  ...
  test("enumerated 2_") {
    val protocol =
      new LabProtocol("enumerated2", "", "", "", 2, false, true, "", 0, "", Nil, ValueList(valueSet123))
    assertResult(6)(protocol.countRuns)
    assertResult("List((foo,1.0)) List((foo,2.0)) List((foo,3.0)) List((foo,1.0)) List((foo,2.0)) List((foo,3.0))")(
      protocol.refElements.mkString(" "))
  }
  test("stepped 1") {
    val protocol = new LabProtocol("", "", "", "", 1, true, true, "", 0, "", Nil, ValueList(valueSet115))
    assertResult(5)(protocol.countRuns)
    assertResult("List((foo,1.0)) List((foo,2.0)) List((foo,3.0)) List((foo,4.0)) List((foo,5.0))")(
      protocol.refElements.mkString(" "))
  }
  test("stepped 2") {
    val protocol = new LabProtocol("stepped2", "", "", "", 10, true, true, "", 0, "", Nil, ValueList(valueSet115))
  ...
  test("avoid floating point error") {
    val protocol = new LabProtocol("stepped3", "", "", "", 1, true, true, "", 0, "", Nil,
                                ValueList(List(new SteppedValueSet("foo", 0.1d, 0.1d, 0.5d))))
    assertResult("List((foo,0.1)) List((foo,0.2)) List((foo,0.3)) List((foo,0.4)) List((foo,0.5))")(
      protocol.refElements.mkString(" "))
  }
  test("both") {
    def make(repetitions:Int) =
      new LabProtocol("both", "", "", "", repetitions, true, true, "", 0, "", Nil,
        ValueList(valueSet123 :+ new SteppedValueSet("bar", 1d, 1d, 5d)))
  ...
  test("both_") {
    def make(repetitions:Int) =
      new LabProtocol("both", "", "", "", repetitions, false, true, "", 0, "", Nil,
        ValueList(valueSet123 :+ new SteppedValueSet("bar", 1d, 1d, 5d)))
    val protocol1 = make(1)
    assertResult(15)(protocol1.countRuns)
    assertResult(
