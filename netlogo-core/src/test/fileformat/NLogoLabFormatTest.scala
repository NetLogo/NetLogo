...
import org.nlogo.api.{ RefEnumeratedValueSet, FileIO, LabProtocol, ValueList }
...
class NLogoLabFormatTest extends NLogoFormatTest[Seq[LabProtocol]] {
  ...
  val expectedProto = LabProtocol("experiment", "setup", "go", "", 10, true, true, "", 0, "not any? fires", List("burned-trees"), ValueList(List(new RefEnumeratedValueSet("density", List(Double.box(40d), Double.box(0.1d), Double.box(70d))))))
