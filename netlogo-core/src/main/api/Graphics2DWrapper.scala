...
class Graphics2DWrapper(g: Graphics2D, renderLabelsAsRectangles: Boolean = false) extends GraphicsInterface {
  ...
  def drawImage(image: java.awt.image.BufferedImage): Unit = { g.drawImage(image, null, 0, 0) }
  ...
  def drawLabel(label: String, x: Double, y: Double, patchSize: Double): Unit = {
    if (renderLabelsAsRectangles) {
      ...
      lines.zipWithIndex.foreach{case (line, i) => g.drawString(line, 0.0f, (1 * fm.getHeight * i).toFloat)}
    }
  }
  ...
  def fill(shape: java.awt.Shape): Unit = { g.fill(shape) }
  def fillRect(x: Int, y: Int, width: Int, height: Int): Unit = { g.fillRect(x, y, width, height) }
  ...
  def setColor(c: java.awt.Color): Unit = { g.setColor(c) }
  ...
  def translate(x: Double, y: Double): Unit = { g.translate(x, y) }
  def drawPolygon(xcors: Array[Int], ycors: Array[Int], length: Int): Unit = { g.drawPolygon(xcors, ycors, length) }
  def fillPolygon(xcors: Array[Int], ycors: Array[Int], length: Int): Unit = { g.fillPolygon(xcors, ycors, length) }
  def drawPolyline(xcors: Array[Int], ycors: Array[Int], length: Int): Unit = { g.drawPolyline(xcors, ycors, length) }
  def dispose(): Unit = { g.dispose() }
