...
trait CompilationManagement extends CoreWorld { this: AgentManagement =>
  ...
  private var oldBreeds: JMap[String, TreeAgentSet] = new JHashMap[String, TreeAgentSet]()
  private var oldLinkBreeds: JMap[String, TreeAgentSet] = new JHashMap[String, TreeAgentSet]()
  ...
  def rememberOldProgram(): Unit = {
    ...
    oldBreeds = new JHashMap[String, TreeAgentSet](breeds)
    oldLinkBreeds = new JHashMap[String, TreeAgentSet](linkBreeds)
  }
  ...
  protected def buildBreedCaches(): Unit = {
    ...
    while (breedIter.hasNext) {
      val breed = breedIter.next().asInstanceOf[TreeAgentSet]
    ...
    while (linkBreedIter.hasNext) {
      val linkBreed = linkBreedIter.next().asInstanceOf[TreeAgentSet]
