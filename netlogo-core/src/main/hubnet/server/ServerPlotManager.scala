...
class ServerPlotManager(workspace: Workspace & HubNetWorkspaceInterface, connectionManager: ConnectionManager,
                        plots: => Seq[PlotInterface], currentPlot: => PlotInterface) extends PlotListener {
  ...
  def setHistogramNumBars(num: Int): Unit = {for(pen <- currentPlot.currentPen) setInterval(pen.state.interval)}
