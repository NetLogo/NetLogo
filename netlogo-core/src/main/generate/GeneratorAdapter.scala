...
class GeneratorAdapter(mv: MethodVisitor, access: Int, name: String, desc: String, igen: Generator#InstructionGenerator[?])
  ...
  override def visitVarInsn(opcode: Int, variable: Int): Unit = { mv.visitVarInsn(opcode, variable) }
  override def visitIincInsn(variable: Int, increment: Int): Unit = { mv.visitIincInsn(variable, increment) }
  override def visitMaxs(maxStack: Int, maxLocals: Int): Unit = { mv.visitMaxs(maxStack, maxLocals) }
  ...
  override def loadArg(arg: Int): Unit = { throw new IllegalStateException }
  override def loadArgs(arg: Int, count: Int): Unit = { throw new IllegalStateException }
  override def loadArgs(): Unit = { throw new IllegalStateException }
  override def loadArgArray(): Unit = { throw new IllegalStateException }
  override def storeArg(arg: Int): Unit = { throw new IllegalStateException }
  override def getLocalType(local: Int): Type = { throw new IllegalStateException }
  override def loadLocal(local: Int): Unit = { throw new IllegalStateException }
  override def storeLocal(local: Int): Unit = { throw new IllegalStateException }
  /**
   * Generates the instruction to load the given local variable on the stack.
   * @param index a local variable index.
   * @param tpe the type of this local variable.
   */
  override def loadLocal(index: Int, tpe: Type): Unit = { mv.visitVarInsn(tpe.getOpcode(ILOAD), index) }
  /**
   * Generates the instruction to store the top stack value in the given local variable.
   * @param index a local variable index.
   * @param tpe the type of this local variable.
   */
  override def storeLocal(index: Int, tpe: Type): Unit = { mv.visitVarInsn(tpe.getOpcode(ISTORE), index) }
  override def loadThis(): Unit = { mv.visitVarInsn(ALOAD, 0) }
  def loadContext(): Unit = { mv.visitVarInsn(ALOAD, 1) }
}
